...
Continuation of an interrupted session: audited the working tree first and preserved all existing uncommitted Group Study / private-beta work (beta allowlist, session enforcement, room server core, AI tools, upload pipeline, client hooks, tests). Filled only the missing layers; nothing was reverted or rebuilt.

**Private beta access control (Phase B, pre-existing, verified):** `src/lib/auth/beta.ts` server-only allowlist (`SCHOLAR_PRIVATE_BETA`, `SCHOLAR_BETA_ALLOWED_EMAILS`, `SCHOLAR_BETA_ALLOWED_USER_IDS` — IDs take precedence; empty config fails closed). Registration is closed server-side at the API, not hidden in UI. Login failures share one generic response regardless of whether the email is unknown, the password is wrong, or the account exists but is not allowlisted (timing-padded scrypt check). Session restore and developer sessions re-check the allowlist on every request, so pre-beta sessions lose access without deleting accounts. Guest Mode untouched. Covered by `tests/private-beta-auth.test.ts` (13 tests).

**New API surface (Phase C/D, this session):** `GET/POST /api/group-study/rooms` (host overview + room creation, code `SCH` + 8 chars from a 32-char unambiguous alphabet, unique-retry); `POST /api/group-study/rooms/join` (code normalize/validate, IP rate limit 30/15min, pending participant with hashed random token cookie, host re-entry); `GET/DELETE /api/group-study/rooms/[roomId]` (snapshot / leave); `POST .../actions` (single validated action union incl. approve/deny/remove/mute/lock/settings/chat/announce/notes/focus/quiz/answer/reveal/poll/vote/resource/page/clear-chat/regenerate-code); `GET .../events` (serverless-safe bounded SSE pushing deduplicated snapshots every 2.5s + `closed` event) and `POST .../events` (presence heartbeat). Existing AI/resource routes from the prior session retained.

**UI (Phase E–K, this session):** `src/components/views/group-study.tsx` — entry page (Join / Host cards, Scholar glass styling, BETA badge), waiting-for-approval screen, ended/removed states, and the room shell with tabs: Overview (stats, announcement, activity), Materials (upload PDF/image/txt, per-room limits, host sets active resource and page, Follow-Host page sync for participants), Group LAM (modes, material-grounded summaries/quizzes), Chat (announcements shown, mute respected), Quiz (host reveal, results), Focus (synchronized timer, pause/resume/end), Notes (revision-guarded, host-controlled editing, copy-out), People (join requests with Allow/Deny, mute/remove, raise hand), Host controls (locks, feature toggles, announcements, end-room confirm, code regenerate/share). Wired into app-shell nav (`Learn → Group Study`, BETA badge) and standalone `/group-study` route with remembered-room reconnection.

**Data model (pre-existing schema, migration this session):** `prisma/migrations/20260916000000_group_study_beta` — additive, idempotent SQL creating six `GroupStudy*` tables + indexes + FKs; no existing table altered; validated against the schema.

**Security posture:** host/participant authorization is enforced server-side inside serialized room transactions (`SELECT … FOR UPDATE`, serializable, retry on `P2034`); participant tokens are 32-byte random values stored only as SHA-256 hashes in room-scoped httpOnly cookies, invalidated on removal/leave/end; room codes are never sequential and never database IDs; join brute force rate-limited per IP with a generic not-found response; every action revalidates role/status/lock/feature flags; uploads re-validated (magic bytes, PDF JS-action rejection, image re-encode, per-room and per-member quotas); AI access requires active room + `aiEnabled` + approved identity, with member/room/host rate limits inside the same lock, and is re-checked after provider calls before publishing. XSS: display names/chat are plain-text rendered (no HTML), control chars rejected by schema.

**Tests:** `tests/group-study-policy.test.ts` (9 tests: role/status gating incl. forged roles, code normalization, name/quiz/action schema forgery) — suite total 204 pass / 0 fail / 9 live skips. `bunx tsc --noEmit` clean; ESLint clean on new/changed paths; `bun run build` passes with all nine group-study routes.

**Deferred (honest status):** realtime voice conference (no RTC infrastructure; UI hooks not faked), collaborative whiteboard (no realtime canvas sync), Google-Docs-level live notes (revision-guarded atomic notes instead), private-per-participant AI responses (current design publishes to room by design), native Android bridge notification events (web contract only), PDF page-level pointer/highlight (page sync only). Android APK rebuild and on-device 320–430px audit not performed in this session (no Android toolchain run): the web UI is responsive down to 320px via the existing stylesheet breakpoints, but device verification and `app-release.apk` remain outstanding — the existing WebView shell and package name are untouched.
