      ...
      return Response.json(
        {
          ...
          message:
            "Enter a study question up to 2,000 characters and valid material context.",
        },
        { status: 400 },
      );
    const { roomId } = await params;
    const result = await runRoomStudyTool(
      roomId,
      `${input.data.mode}: ${input.data.prompt}`,
      input.data.mode === "quiz-us" ? "quiz" : "text",
      request.signal,
