...
export function LiveTutorView() {
  ...
  const sendMessage = useCallback(async (raw: string, inputMode: "text" | "voice" = "text") => {
    ...
    if (/\b(save|add) (this|that|the answer) (as|to) (a )?note(s)?\b/i.test(message)) {
      ...
      setPendingAction({ type: "create-note", title: `Live Tutor — ${context.chapterTitle ?? context.subjectTitle ?? "study note"}`, content: latestAnswer }); setStatus("waiting-confirmation"); return;
    }
  ...
  return <section className={`lt-root lt-${personality} ${switching ? "is-switching" : ""}`} data-personality={personality} aria-label="LAM Live Tutor">
    <PersonalityBackground personality={personality} /><div className="lt-environment-scrim" aria-hidden="true" />
    <nav className="lt-nav" aria-label="Live Tutor navigation"><span className="lt-nav-brand"><span className="lt-scholar-mark"><BrainCircuit /></span>Scholar<span className="lt-nav-beta">LIVE TUTOR · BETA</span></span><span className="lt-nav-div" /><div className="lt-nav-tabs" role="group" aria-label="Personality">{LIVE_TUTOR_PERSONALITIES.map((item) => <button key={item} className="lt-nav-tab" aria-pressed={personality === item} onClick={() => switchPersonality(item)}>{PERSONALITY_COPY[item].shortLabel}</button>)}</div><span className="lt-nav-div" /><button className="lt-nav-new" onClick={newConversation} aria-label="New conversation"><Plus /></button></nav>
    ...
    <div className="lt-canvas">{messages.length || streaming ? <div className="lt-canvas-scroll" ref={scrollRef} role="log" aria-live="polite" aria-label="Live Tutor conversation"><div className="lt-convo-col">
      {captions && messages.map((message) => <article className="lt-turn" data-role={message.role} key={message.id}><div className="lt-turn-meta">{message.role === "assistant" ? <><span className="lt-turn-orb"><Sparkles /></span>{copy.label}</> : <>You{message.inputMode === "voice" ? " · voice" : ""}</>}</div><div className="lt-bubble">{message.role === "assistant" ? <LamResponse content={message.content} optimized /> : message.content}</div>{message.role === "assistant" ? <div className="lt-msg-actions"><button className="lt-msg-act" onClick={() => void navigator.clipboard.writeText(message.content)}><Copy />Copy</button><button className="lt-msg-act" onClick={() => void remember(message.content)}><MemoryStick />Remember</button><button className="lt-msg-act" onClick={() => { setPendingAction({ type: "create-note", title: `Live Tutor — ${context.chapterTitle ?? context.subjectTitle ?? "study note"}`, content: message.content }); setStatus("waiting-confirmation"); }}><FileText />Save to Notes</button></div> : null}</article>)}
    ...
    {!prepared ? <div className="lt-prep" role="dialog" aria-label="Preparing Live Tutor memory"><div className="lt-prep-card"><div className="lt-prep-title"><span className="lt-prep-spinner" />Preparing Live Tutor…</div><div className="lt-prep-step done"><span className="chk"><Check /></span>Verifying your Scholar session</div><div className="lt-prep-step done"><span className="chk"><Check /></span>Reading recent learning activity</div><div className="lt-prep-step"><span className="chk" />Restoring personality sessions</div></div></div> : null}
  </section>;
}
