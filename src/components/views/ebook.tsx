...
import {
  ...
  ChevronDown, Clock, Zap, FileText, Sparkles, Loader2, X, ListChecks,
  ZoomIn, ZoomOut, RotateCw, Maximize2, Minimize2, PanelLeftClose, PanelLeftOpen,
  Settings2, PenLine, Tag, CheckCircle2, Eye, Upload, BookMarked,
  RefreshCw, FileStack, AlertCircle, Loader, ArrowRight, Save,
} from "lucide-react";
...
const BOOKS: EbookBook[] = [
  {
    ...
    pageDir: "ebook-pages",
    chapters: [
      { id: "ch1", title: "Units and Measurement", startPage: 1, endPage: 45, color: "#3b82f6" },
      { id: "ch2", title: "Motion in a Straight Line", startPage: 46, endPage: 90, color: "#10b981" },
    ],
  },
  {
    id: "maths-pt1",
...
const PAGE_TAGS = ["Theory", "Formula", "Example", "Solved Question", "Homework", "Exercise", "Important", "Difficult", "Revision", "Doubt", "Diagram", "Derivation"];
...
export function EBookView() {
  ...
  // Persist
  const persist = useCallback((updates: Partial<{ notes: Record<number, PageNote>; bookmarks: PageBookmark[]; ocrReviewed: Record<number, string>; lastPage: number }>) => {
    const data = loadData() ?? {};
  ...
  const saveNote = useCallback(() => {
    ...
    const note: PageNote = { page: noteModal, text: noteText, tags: noteTags, createdAt: Date.now() };
  ...
  const runOCR = useCallback(async (page: number) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "OCR failed. Try again.";
  ...
  // AI Explain
  const handleAIExplain = useCallback(async (page: number) => {
  ...
  const searchResults = useMemo(() => {
    ...
    const results: { type: string; label: string; page: number; snippet: string }[] = [];
    // Search bookmarks
    bookmarks.forEach(bm => {
      if (bm.title.toLowerCase().includes(q) || `pg ${bm.page}`.includes(q)) {
        results.push({ type: "bookmark", label: bm.title, page: bm.page, snippet: `Bookmark on Pg ${bm.page}` });
      }
    });
    // Search notes
    Object.values(notes).forEach(note => {
      if (note.text.toLowerCase().includes(q) || note.tags.some(t => t.toLowerCase().includes(q))) {
        results.push({ type: "note", label: `Pg ${note.page} note`, page: note.page, snippet: note.text.slice(0, 80) });
      }
    });
    // Search reviewed OCR
    Object.entries(ocrReviewed).forEach(([pageStr, text]) => {
      if (text.toLowerCase().includes(q)) {
        ...
        results.push({ type: "ocr", label: `Pg ${page} (OCR)`, page, snippet: text.substring(Math.max(0, idx - 30), idx + q.length + 30) });
      }
    });
  ...
  const getChapter = (page: number) => chapters.find(c => page >= c.startPage && page <= c.endPage);
      ...
      <div className="relative min-h-[calc(100vh-4rem)] bg-[#0a0a0f] overflow-hidden -m-4 lg:-m-6 text-white eb-font">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <motion.nav initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between py-4">
            ...
            <Button size="sm" variant="outline" onClick={() => setMapperOpen(true)} className="bg-white/5 border-white/15 text-white hover:bg-white/10">
          ...
          <div className="mt-8 mb-8">
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-1.5 eb-glass rounded-full px-3 py-1 text-xs text-white/50 mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            {[
              { icon: FileStack, label: "Pages", value: totalPages, accent: "#6366f1" },
              { icon: BookOpen, label: "Chapters", value: chapters.length, accent: "#10b981" },
              { icon: Bookmark, label: "Bookmarks", value: totalBookmarks, accent: "#f59e0b" },
              { icon: PenLine, label: "Notes", value: totalNotes, accent: "#0ea5e9" },
              { icon: Eye, label: "OCR Reviewed", value: totalOcrReviewed, accent: "#a855f7" },
            ].map((s, i) => (
              <div key={i} className="eb-glass rounded-xl p-3 flex items-center gap-3">
                <div className="grid place-items-center h-9 w-9 rounded-lg" style={{ background: `${s.accent}15` }}>
                  <s.icon className="h-4 w-4" style={{ color: s.accent }} />
                </div>
                <div>
                  <p className="text-lg font-bold text-white leading-none">{s.value}</p>
                  <p className="text-[10px] uppercase tracking-wide text-white/40 mt-0.5">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Continue Reading */}
          <div className="eb-glass rounded-2xl p-4 mb-6 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setView("reader")}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-wide text-white/40">Continue Reading</p>
                <p className="text-sm font-semibold text-white">Pg {activePage} — {getChapter(activePage)?.title ?? "Unassigned"}</p>
                <p className="text-xs text-white/50">{progress}% through book</p>
              </div>
            </div>
          ...
          <div className="eb-glass rounded-2xl p-4 mb-6">
            <div className="relative mb-2">
              ...
              <Input placeholder="Search bookmarks, notes, and reviewed OCR text..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-white/5 border-white/10 text-white placeholder:text-white/40" />
            </div>
            <p className="text-[10px] text-white/30 mb-2">Search works on manually added bookmarks, notes, tags, and reviewed OCR only. Unreviewed pages are not searched.</p>
            {searchResults.length > 0 && (
              <div className="space-y-2 max-h-48 overflow-y-auto eb-scroll">
                {searchResults.map((r, i) => (
                  <div key={i} className="p-2 rounded-lg bg-white/5 border border-white/10 text-xs cursor-pointer hover:bg-white/10" onClick={() => { jumpTo(r.page); setView("reader"); }}>
                    <div className="flex items-center gap-2 mb-0.5">
                      <Badge className="text-[9px] px-1.5 py-0" variant="secondary">{r.type}</Badge>
                      <span className="text-white/80 font-medium">{r.label}</span>
                    </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {BOOKS.map((book) => (
              <button
                key={book.id}
                onClick={() => { setActiveBookId(book.id); setActivePage(1); localStorage.setItem("scholar:ebook:last-book", book.id); }}
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <BookOpen className="h-4 w-4" style={{ color: book.chapters[0]?.color ?? "#888" }} />
                  <span className="text-sm font-semibold text-white">{book.title}</span>
                </div>
                <p className="text-[11px] text-white/40">{book.subject} · {book.totalPages} pages · {book.chapters.length} chapters</p>
              </button>
            ))}
          </div>

          {/* Chapters */}
          <p className="text-sm text-white/50 mb-3">Chapters — {activeBook.title}</p>
          <div className="grid grid-cols-1 gap-3 mb-8">
            {chapters.map((ch, i) => {
              const chPages = ch.endPage - ch.startPage + 1;
              const chBookmarks = bookmarks.filter(b => b.page >= ch.startPage && b.page <= ch.endPage).length;
              const chNotes = Object.values(notes).filter(n => n.page >= ch.startPage && n.page <= ch.endPage).length;
              const chOcr = Object.keys(ocrReviewed).map(Number).filter(p => p >= ch.startPage && p <= ch.endPage).length;
              return (
                <motion.div key={ch.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  className="eb-glass rounded-2xl p-5 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => { jumpTo(ch.startPage); setView("reader"); }}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <p className="text-[11px] uppercase tracking-wide font-medium mb-1" style={{ color: ch.color }}>Chapter {i + 1}</p>
                      <h3 className="text-lg font-semibold text-white">{ch.title}</h3>
                      <div className="flex items-center gap-3 mt-2 text-[11px] text-white/40">
                        <span className="flex items-center gap-1"><FileText className="h-3 w-3" /> Pg {ch.startPage}–{ch.endPage} ({chPages} pages)</span>
                        <span className="flex items-center gap-1"><Bookmark className="h-3 w-3" /> {chBookmarks} bookmarks</span>
                        <span className="flex items-center gap-1"><PenLine className="h-3 w-3" /> {chNotes} notes</span>
                        <span className="flex items-center gap-1"><Eye className="h-3 w-3" /> {chOcr} OCR'd</span>
                      </div>
                    </div>
                    <div className="grid place-items-center h-12 w-12 rounded-xl border border-white/10 shrink-0" style={{ background: `${ch.color}15` }}>
                      <BookOpen className="h-6 w-6" style={{ color: ch.color }} />
                    </div>
                  </div>
                </motion.div>
              );
          ...
          <div className="eb-glass rounded-xl p-3 flex items-center gap-3 text-xs text-white/50">
            ...
            <span>Source: <strong className="text-white/70">{activeBook.title}</strong> · {totalPages} scanned pages · Rendered as page images · OCR optional</span>
          </div>
        </div>
  ...
  return (
    <div className={`relative ${fullscreen ? "fixed inset-0 z-50" : "min-h-[calc(100vh-4rem)]"} bg-[#0a0a0f] overflow-hidden ${fullscreen ? "" : "-m-4 lg:-m-6"} text-white eb-font flex`}>
      <style>{EB_STYLE}</style>
      ...
      {showSidebar && !fullscreen && (
        <aside className="hidden lg:flex flex-col w-64 border-r border-white/10 bg-black/40 backdrop-blur-xl shrink-0 h-[calc(100vh-4rem)] sticky top-0">
          <div className="p-4 border-b border-white/10">
            <button onClick={() => setView("home")} className="flex items-center gap-2 text-sm text-white/60 hover:text-white">
              <ChevronLeft className="h-4 w-4" /> Library
            </button>
          </div>
          <ScrollArea className="flex-1 px-2">
            {/* Chapters */}
            <p className="text-[10px] uppercase tracking-wide text-white/40 px-2 py-2">Chapters</p>
            {chapters.map(ch => (
              <button key={ch.id} onClick={() => jumpTo(ch.startPage)}
                ...
                <span className="text-[10px] text-white/30">Pg {ch.startPage}–{ch.endPage}</span>
              </button>
            ))}
            {/* Page grid */}
            <p className="text-[10px] uppercase tracking-wide text-white/40 px-2 py-2 mt-3">Pages</p>
            <div className="grid grid-cols-5 gap-1 px-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(pg => {
                ...
                return (
                  <button key={pg} onClick={() => jumpTo(pg)}
                    ...
                    style={ch && pg !== activePage ? { borderBottom: `2px solid ${ch.color}40` } : {}}>
                    {pg}
                    {hasBm && <span className="absolute top-0 right-0 w-1.5 h-1.5 rounded-full bg-amber-400" />}
                    {hasNote && <span className="absolute bottom-0 right-0 w-1.5 h-1.5 rounded-full bg-sky-400" />}
                    {hasOcr && <span className="absolute top-0 left-0 w-1.5 h-1.5 rounded-full bg-purple-400" />}
                  </button>
                );
              })}
            </div>
            ...
            {bookmarks.length > 0 && (
              <>
                <p className="text-[10px] uppercase tracking-wide text-white/40 px-2 py-2 mt-3">Bookmarks</p>
                {bookmarks.slice(0, 10).map(bm => (
                  <div key={bm.id} className="text-xs text-white/50 px-3 py-1.5 rounded hover:bg-white/5 cursor-pointer flex items-center gap-1.5" onClick={() => jumpTo(bm.page)}>
          ...
          <button onClick={() => setShowSidebar(false)} className="p-3 border-t border-white/10 text-xs text-white/40 hover:text-white flex items-center gap-1">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="sticky top-0 z-20 bg-black/60 backdrop-blur-xl border-b border-white/10 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            {!showSidebar && !fullscreen && <button onClick={() => setShowSidebar(true)} className="text-white/50 hover:text-white"><PanelLeftOpen className="h-4 w-4" /></button>}
            <span className="text-xs text-white/50">Pg {activePage}</span>
            {currentChapter && <Badge className="text-[9px] px-2 py-0" style={{ background: `${currentChapter.color}20`, color: currentChapter.color, border: `1px solid ${currentChapter.color}40` }}>{currentChapter.title}</Badge>}
            {pageNote && <Badge className="text-[9px] px-2 py-0 bg-sky-500/20 text-sky-300">Has note</Badge>}
            {hasOCR && <Badge className="text-[9px] px-2 py-0 bg-purple-500/20 text-purple-300">OCR'd</Badge>}
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={() => setZoom(Math.max(0.5, zoom - 0.1))} className="text-white/50 hover:text-white p-1.5 rounded-lg hover:bg-white/5"><ZoomOut className="h-4 w-4" /></button>
            <span className="text-xs text-white/40 w-10 text-center">{Math.round(zoom * 100)}%</span>
            <button onClick={() => setZoom(Math.min(3, zoom + 0.1))} className="text-white/50 hover:text-white p-1.5 rounded-lg hover:bg-white/5"><ZoomIn className="h-4 w-4" /></button>
            ...
            <button onClick={() => { setOcrModal(activePage); setOcrText(ocrReviewed[activePage] ?? ""); }} className="text-white/50 hover:text-white p-1.5 rounded-lg hover:bg-white/5"><Eye className="h-4 w-4" /></button>
            <button onClick={() => handleAIExplain(activePage)} className="text-white/50 hover:text-white p-1.5 rounded-lg hover:bg-white/5"><Sparkles className="h-4 w-4" /></button>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activePage} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}
              ...
              <div className="mb-3 flex items-center gap-2">
                <span className="text-sm font-semibold text-white/80">Pg {activePage}</span>
                {currentChapter && <span className="text-xs text-white/40">— {currentChapter.title}</span>}
              </div>
              {/* Page Image */}
              <div className="relative eb-glass rounded-2xl overflow-hidden shadow-2xl" style={{ maxWidth: `${zoom * 100}%`, transition: "max-width 0.2s" }}>
                <img src={pageImage(activePage, pageDir)} alt={`Page ${activePage}`} className="w-full h-auto block" style={{ transform: `rotate(${rotation}deg)`, transition: "transform 0.3s" }} loading="lazy" />
              </div>
              {/* Page Actions */}
              <div className="mt-4 flex flex-wrap gap-2 justify-center">
                <Button size="sm" variant="outline" onClick={() => toggleBookmark(activePage)} className="bg-white/5 border-white/15 text-white hover:bg-white/10 text-xs">
                  {isBookmarked ? <><BookmarkCheck className="h-3.5 w-3.5 mr-1.5 text-amber-400" /> Bookmarked</> : <><Bookmark className="h-3.5 w-3.5 mr-1.5" /> Bookmark</>}
                </Button>
                <Button size="sm" variant="outline" onClick={() => { setPageNoteModal(activePage); setNoteText(pageNote?.text ?? ""); setNoteTags(pageNote?.tags ?? []); }} className="bg-white/5 border-white/15 text-white hover:bg-white/10 text-xs">
                  <PenLine className="h-3.5 w-3.5 mr-1.5" /> {pageNote ? "Edit Note" : "Add Note"}
                </Button>
                <Button size="sm" variant="outline" onClick={() => { setOcrModal(activePage); setOcrText(ocrReviewed[activePage] ?? ""); }} className="bg-white/5 border-white/15 text-white hover:bg-white/10 text-xs">
                  <Eye className="h-3.5 w-3.5 mr-1.5" /> {hasOCR ? "View OCR" : "Extract Text (OCR)"}
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleAIExplain(activePage)} className="bg-white/5 border-white/15 text-white hover:bg-white/10 text-xs">
                  <Sparkles className="h-3.5 w-3.5 mr-1.5" /> AI Explain
                </Button>
                <Button size="sm" variant="outline" onClick={() => { addNote({ title: `Physics E-Book Pg ${activePage}`, content: `Page ${activePage} of Physics textbook. Chapter: ${currentChapter?.title ?? "Unknown"}. ${pageNote?.text ?? ""}`, folder: "E-Book", color: "indigo", tags: ["ebook", "physics", `pg-${activePage}`] }); toast.success("Saved to Notes"); }} className="bg-white/5 border-white/15 text-white hover:bg-white/10 text-xs">
              ...
              {aiExplainPage === activePage && (aiLoading || aiExplanation) && (
                <div className="mt-4 max-w-2xl w-full eb-glass rounded-xl p-4">
                  ...
                  {aiExplanation && <div className="text-sm text-white/80 whitespace-pre-wrap" style={{ lineHeight: 1.6 }}>{aiExplanation}</div>}
                </div>
              )}
              {/* Page Note Preview */}
              {pageNote && (
                <div className="mt-4 max-w-2xl w-full eb-glass rounded-xl p-4">
                  ...
                  <p className="text-sm text-white/70 whitespace-pre-wrap">{pageNote.text}</p>
                  {pageNote.tags.length > 0 && (
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {pageNote.tags.map(t => <Badge key={t} className="text-[9px] px-1.5 py-0 bg-white/10 text-white/60">{t}</Badge>)}
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="sticky bottom-0 bg-black/60 backdrop-blur-xl border-t border-white/10 px-4 py-2 flex items-center justify-between">
          <Button size="sm" variant="ghost" onClick={goPrev} disabled={activePage === 1} className="text-white/60 hover:text-white">
            <ChevronLeft className="h-4 w-4 mr-1" /> Prev
          </Button>
          <div className="flex items-center gap-2">
            <input type="number" min={1} max={totalPages} value={activePage} onChange={(e) => jumpTo(parseInt(e.target.value) || 1)}
          ...
          <Button size="sm" variant="ghost" onClick={goNext} disabled={activePage === totalPages} className="text-white/60 hover:text-white">
      ...
      <Dialog open={noteModal !== null} onOpenChange={(o) => !o && setPageNoteModal(null)}>
        <DialogContent className="eb-glass-strong max-w-lg text-white border-white/10">
          <DialogHeader>
            <DialogTitle className="text-white">Note for Pg {noteModal}</DialogTitle>
            <DialogDescription className="text-white/50">Add personal notes, important points, doubts, or homework reminders for this page.</DialogDescription>
          </DialogHeader>
          <textarea value={noteText} onChange={(e) => setNoteText(e.target.value)} rows={5} placeholder="Write your note..." className="w-full p-3 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/30 resize-none" />
          <div>
          ...
          <DialogFooter>
            <DialogClose asChild><Button variant="ghost" size="sm" className="text-white hover:bg-white/10">Cancel</Button></DialogClose>
            <Button size="sm" onClick={saveNote} className="bg-indigo-500 text-white hover:bg-indigo-600">Save Note</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* OCR Modal */}
      <Dialog open={ocrModal !== null} onOpenChange={(o) => !o && setOcrModal(null)}>
        <DialogContent className="eb-glass-strong max-w-4xl text-white border-white/10 max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="text-white">OCR Text Extraction — Pg {ocrModal}</DialogTitle>
            <DialogDescription className="text-white/50">Extract text from this scanned page. You can review and edit the result before saving.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 overflow-hidden">
            {/* Original scan */}
            <div className="rounded-xl overflow-hidden border border-white/10 max-h-[50vh] overflow-y-auto eb-scroll">
              {ocrModal && <img src={pageImage(ocrModal, pageDir)} alt={`Page ${ocrModal}`} className="w-full" />}
            </div>
            {/* Extracted text */}
            <div className="flex flex-col gap-2">
              <Button size="sm" onClick={() => ocrModal && runOCR(ocrModal)} disabled={ocrLoading} className="bg-purple-500 text-white hover:bg-purple-600">
                {ocrLoading ? <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> Running OCR...</> : <><Eye className="h-3.5 w-3.5 mr-1.5" /> Run OCR</>}
              </Button>
              <textarea value={ocrText} onChange={(e) => setOcrText(e.target.value)} rows={15} placeholder="Click 'Run OCR' to extract text, then edit as needed..." className="w-full flex-1 p-3 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/30 resize-none eb-scroll" />
            </div>
          </div>
          <DialogFooter>
            <DialogClose asChild><Button variant="ghost" size="sm" className="text-white hover:bg-white/10">Close</Button></DialogClose>
            <Button size="sm" onClick={saveOCR} disabled={!ocrText} className="bg-emerald-500 text-white hover:bg-emerald-600">Save Reviewed Text</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function ChapterMapper({ open, onOpenChange, chapters, onSave }: {
  open: boolean;
  ...
  useEffect(() => {
    let cancelled = false;
    queueMicrotask(() => { if (!cancelled) setLocal(chapters); });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="eb-glass-strong max-w-lg text-white border-white/10">
        <DialogHeader>
          ...
          <DialogDescription className="text-white/50">Assign page ranges to chapters. Unassigned pages will be hidden.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          {local.map((ch, i) => (
            <div key={ch.id} className="eb-glass rounded-xl p-3 space-y-2">
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full" style={{ background: ch.color }} />
                <Input value={ch.title} onChange={(e) => setLocal(prev => prev.map((c, j) => j === i ? { ...c, title: e.target.value } : c))} className="bg-white/5 border-white/10 text-white text-sm flex-1" />
              </div>
              <div className="flex items-center gap-2 text-xs text-white/60">
                <span>Start:</span>
                <Input type="number" min={1} max={TOTAL_PAGES} value={ch.startPage} onChange={(e) => setLocal(prev => prev.map((c, j) => j === i ? { ...c, startPage: parseInt(e.target.value) || 1 } : c))} className="w-16 bg-white/5 border-white/10 text-white text-center" />
                <span>End:</span>
                <Input type="number" min={1} max={TOTAL_PAGES} value={ch.endPage} onChange={(e) => setLocal(prev => prev.map((c, j) => j === i ? { ...c, endPage: parseInt(e.target.value) || 1 } : c))} className="w-16 bg-white/5 border-white/10 text-white text-center" />
                <span className="text-white/40">({ch.endPage - ch.startPage + 1} pages)</span>
              </div>
            </div>
          ))}
          <Button size="sm" variant="outline" onClick={() => setLocal([...local, { id: `ch${local.length + 1}`, title: `Chapter ${local.length + 1}`, startPage: 1, endPage: 10, color: "#6366f1" }])} className="bg-white/5 border-white/15 text-white hover:bg-white/10 w-full">
            + Add Chapter
          </Button>
        </div>
        <DialogFooter>
          <DialogClose asChild><Button variant="ghost" size="sm" className="text-white hover:bg-white/10">Cancel</Button></DialogClose>
          <Button size="sm" onClick={() => onSave(local)} className="bg-indigo-500 text-white hover:bg-indigo-600">Save Mapping</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
