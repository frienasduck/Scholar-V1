...
import {
  AlertCircle, ArrowRight, BarChart3, Check, Copy, FileText, Hash, MessageSquare, Orbit, Sparkles, Users,
} from "lucide-react";
import { groupRequest, errorMessage, formatRoomCode, copyRoomCode, type CreatedStudyRoom } from "@/components/group-study/client";
...
function JoinForm({ canHost, onHost, onJoined }: { canHost: boolean | null; onHost: () => void; onJoined: (roomId: string) => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const known: Record<string, { title: string; detail: string }> = {
        "That study room couldn't be found or is unavailable.": { title: "Study room not found", detail: "Check the code and try again." },
  ...
  return (
    <form className="card__form" noValidate onSubmit={submit} id="join">
      ...
      {canHost ? <button type="button" className="btn btn--glass" onClick={onHost}>Create Study Room <ArrowRight aria-hidden="true" /></button> : canHost === false ? <Link href="/?signin=1" className="btn btn--glass" role="button">
        Sign in as host
      </Link> : null}
...
function CreateForm({ onCreated, onBack }: { onCreated: (room: CreatedStudyRoom) => void; onBack: () => void }) {
  const [name, setName] = useState("");
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const room = await groupRequest<CreatedStudyRoom>("/api/group-study/rooms", { method: "POST", body: JSON.stringify({ name, subject, topic }) });
      if (!room.ok || !room.roomId || !room.code) throw new Error("The room response was incomplete. Please check your connection and try again.");
  ...
  return <form className="card__form" onSubmit={submit}>
    {[{ label: "Room name", value: name, update: setName, limit: 80, placeholder: "Physics Revision", required: true }, { label: "Subject (optional)", value: subject, update: setSubject, limit: 80, placeholder: "Physics" }, { label: "Topic (optional)", value: topic, update: setTopic, limit: 160, placeholder: "Work, Energy and Power" }].map((field, i) => <div className="card__field" key={field.label}>
      <label className="card__label" htmlFor={`gs-create-${i}`}>{field.label}</label>
      <div className="card__control"><input className="card__input" style={{ paddingLeft: 16 }} id={`gs-create-${i}`} value={field.value} onChange={(event) => field.update(event.target.value)} maxLength={field.limit} minLength={field.required ? 2 : undefined} required={field.required} placeholder={field.placeholder} /></div>
    </div>)}
    {error ? <FormStatus title="Couldn't create the room" detail={error} /> : null}
    <button className="btn btn--primary" type="submit" disabled={busy || name.trim().length < 2}>{busy ? "Creating…" : "Create Study Room"} <ArrowRight aria-hidden="true" /></button>
    <button className="btn btn--glass" type="button" disabled={busy} onClick={onBack}>Back to join</button>
  </form>;
}

function RoomCreated({ room, onEnter }: { room: CreatedStudyRoom; onEnter: () => void }) {
  const [copied, setCopied] = useState(false);
  const [error, setError] = useState("");
  const copy = async () => {
    setError("");
    try { await copyRoomCode(room.code); setCopied(true); }
    catch (cause) { setError(errorMessage(cause)); }
  };
  return <div className="card__form" role="region" aria-label="Room created">
    <p className="card__label">ROOM CODE</p>
    <code className="card__room-code" aria-label={`Room code ${formatRoomCode(room.code)}`}>{formatRoomCode(room.code)}</code>
    <p className="card__support">Share this code with your study group.</p>
    <button type="button" className="btn btn--glass" onClick={() => void copy()}>{copied ? <Check aria-hidden="true" /> : <Copy aria-hidden="true" />}{copied ? "Code copied" : "Copy Code"}</button>
    <span role="status" className="card__note">{copied ? "Room code copied to clipboard." : ""}</span>
    {error ? <FormStatus title="Copy unavailable" detail={error} /> : null}
    <button type="button" className="btn btn--primary" onClick={onEnter}>Enter Study Room <ArrowRight aria-hidden="true" /></button>
  </div>;
}
...
export function GroupStudyLanding() {
  ...
  const [overview, setOverview] = useState<{ canHost: boolean; roomId?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const idle = window.setTimeout(() => { setRoomId(saved); setReady(true); }, 0);
    return () => window.clearTimeout(idle);
  }, []);

  const checkAccess = useCallback(async (signal?: AbortSignal) => {
    try {
      const value = await groupRequest<{ canHost: boolean; roomId?: string }>("/api/group-study/rooms", { signal });
      if (!signal?.aborted) { setOverview(value); setAccessError(""); }
    } catch { if (!signal?.aborted) setAccessError("Couldn't check hosting access. Try again."); }
  }, []);
  useEffect(() => {
  ...
  const enter = useCallback((id: string) => {
    rememberRoomId(id);
    window.history.replaceState(window.history.state, "", `/group-study?room=${encodeURIComponent(id)}`);
    setRoomId(id); setReady(true);
  }, []);
        ...
        <main className="hero__body">
          ...
          <div className="cards">
            <section className="card gs-glass-card" aria-labelledby="gs-join-title">
              ...
              <h2 className="card__title" id="gs-join-title">{created ? "Your study room is ready" : form === "create" ? "Host a study room" : "Join a study room"}</h2>
              <p className="card__support">{created ? created.name : form === "create" ? "Create a room and invite your group. Your host access is already active." : "Enter your name and the study code your host shared with you."}</p>
              {created ? <RoomCreated room={created} onEnter={() => enter(created.roomId)} /> : form === "create" && overview?.canHost ? <CreateForm onCreated={setCreated} onBack={() => setForm("join")} /> : <JoinForm canHost={overview?.canHost ?? null} onHost={() => setForm("create")} onJoined={enter} />}
              {accessError ? <><FormStatus title="Connection issue" detail={accessError} /><button type="button" className="btn btn--glass" onClick={() => void checkAccess()}>Retry access check</button></> : null}
              {!created && overview?.roomId ? <button type="button" className="btn btn--glass" onClick={() => enter(overview.roomId!)}>Resume active room</button> : null}
            </section>
