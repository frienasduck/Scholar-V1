...
export async function groupRequest<T>(url: string, init: RequestInit = {}, timeoutMs = 15_000): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      const fallback = response.status === 401 ? "Your room session has expired. Join again to continue."
    ...
    if (controller.signal.aborted) throw new GroupStudyClientError("The request took too long. Please try again.", 408);
    throw new GroupStudyClientError("Connection lost. Check your connection and try again.", 0);
  } finally {
