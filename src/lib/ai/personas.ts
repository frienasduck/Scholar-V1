...
const PERSONAS: Record<string, string> = {
  ...
  "memory-predictor": "Suggest revision priorities from supplied evidence. Never invent review dates or claim to measure memory retention when timestamps and review outcomes are unavailable.",
...
const MODE_INSTRUCTIONS: Partial<Record<AIMode, string>> = {
  ...
  "mock-exam": "Return a JSON object with a questions array. Every question requires id (string), question (string), type (mcq, short or long), marks (positive number up to 20), chapterId (string), and chapterTitle (string). MCQs require exactly four string options and correctAnswer as a zero-based option index. Short and long questions require modelAnswer (string). Use the supplied chapter context; no prose outside JSON.",
  "answer-evaluation": "Return a JSON object with score (nonnegative number), maxScore (positive number), correctConcepts and missingConcepts (arrays of strings), formulaFeedback, unitsFeedback, stepFeedback and improvedAnswer (nonempty strings), and markingBreakdown (array of {criterion: string, marksAwarded: nonnegative number, maxMarks: positive number, feedback: string}). score cannot exceed maxScore; marksAwarded cannot exceed maxMarks. Do not replace these fields with a generic feedback field.",
