import { z } from "zod";
import { GROUP_FEATURE_IDS } from "./features";
...
export const quizQuestionSchema = z
  ...
  .strict()
  .refine(
    (value) => value.correctAnswer < value.options.length,
...
export const groupActionSchema = z.discriminatedUnion("action", [
    ...
    .object({
      ...
      settings: z
        .object({
          ...
          participantUploads: z.boolean().optional(),
          notesEditable: z.boolean().optional(),
          followHost: z.boolean().optional(),
        })
        .strict(),
    })
    .strict(),
  z
    .object({
      ...
      notesRevision: z.number().int().nonnegative().optional(),
    })
    .strict(),
  z
    .object({
      action: z.literal("clear-hand"),
      participantId: z.string().min(1).max(80),
    })
    .strict(),
  z
    .object({
      ...
      operation: z.enum(["start", "pause", "resume", "stop"]),
  ...
  z
    .object({
      ...
      title: z.string().trim().min(1).max(120).default("Room quiz"),
  ...
  z
    ...
    .strict(),
  z
    .object({ action: z.literal("reveal"), quizId: z.string().min(1).max(80) })
    .strict(),
  z
    .object({
  ...
  z
    .object({
      ...
      resourceId: z.string().max(80).nullable(),
  ...
  z
    .object({
      action: z.literal("feature-policy"),
      feature: z.enum(GROUP_FEATURE_IDS),
      access: z.enum(["off", "host", "group"]),
    })
    .strict(),
  z
    .object({
      action: z.literal("navigation-mode"),
      mode: z.enum(["follow", "guided", "open"]),
    })
    .strict(),
  z
    .object({
      action: z.literal("session-details"),
      name: z.string().trim().min(2).max(80),
  ...
  z
    ...
    .strict(),
  z
    .object({ action: z.literal("follow-host"), following: z.boolean() })
    .strict(),
  z
    .object({
      action: z.literal("session-path"),
      steps: z
        .array(
          z
            .object({
  ...
  z
    .object({
      action: z.literal("media-state"),
      voiceJoined: z.boolean(),
      cameraActive: z.boolean(),
    })
    .strict(),
  z
    .object({
      action: z.literal("media-policy"),
      voiceEnabled: z.boolean().optional(),
      cameraEnabled: z.boolean().optional(),
      reactionsEnabled: z.boolean().optional(),
      participantId: z.string().max(80).optional(),
...
export function canPerformAction(
  ...
  if (status !== "approved")
    return action === "heartbeat" || action === "leave";
